...
$this->title = 'Journal Debit Report (Ledger Wise)';
...
<div class="card">
    <div class="card-bofy m-2">
        ...
        <?=
        $this->render('_debit-ledg-search', [
            'request' => $request,
            'ledgers' => $ledgers
        ]);
        ?>
        ...
        <table id="report" class="table dataTable">
            ...
            <tfoot>
                <tr>
                    <td colspan="5">&nbsp;</td>
                    <td class="grand-tot" colspan="2"><label for="tot_header">Total Debit:</label></td>
        ...
        $('#report').DataTable({
            layout: {
                topStart: {
                    buttons: [{
                            extend: 'copyHtml5',
                            title: 'Journal Debit Report (Ledger Wise)' + ($('#ledger').val() != '' ? ' - ' + $('#ledger').val() : '') + (($('#from').val() != '' && $('#to').val() != '') ? '\n From: ' + $('#from').val() + ' - To: ' + $('#to').val() : ''),
                        },
                        {
                            extend: 'csvHtml5',
                            title: 'Journal Debit Report (Ledger Wise)' + ($('#ledger').val() != '' ? ' - ' + $('#ledger').val() : '') + (($('#from').val() != '' && $('#to').val() != '') ? '\n From: ' + $('#from').val() + ' - To: ' + $('#to').val() : ''),
