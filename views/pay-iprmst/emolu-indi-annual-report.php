<?php
$this->title = 'Emolument List - Individual';
...
<div class="card">
    <div class="card-bofy m-2">
        ...
        <?=
        $this->render('_emolu-indi-annual-search', [
            'request' => $request,
            'payFields' => $payFields
        ]);
        ?>
        ...
        if (!empty($empUPFNo) && !empty($year) && !empty($fldCode)) { ?>
            ...
            <div id="ifrmDv" style="text-align: center;">
                <iframe id="ifrm" src="emolu-indi-annual-report-pdf?empUPFNo=<?= $empUPFNo; ?>&year=<?= $year; ?>&fldCode=<?= $fldCode; ?>" style="width: 90%; height: 650px;" type="application/pdf"></iframe>
            </div>
        <?php } ?>
    </div>
</div>
