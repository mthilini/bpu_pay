...
use app\models\PayIncomeSearch;
use Yii;
...
/**
 * PayIncomeController implements the CRUD actions for PayIncome model.
 */
class PayIncomeController extends Controller
{
    ...
    /**
     * Lists all PayIncome models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new PayIncomeSearch();
    ...
    /**
     * Updates an existing PayIncome model.
    ...
    /**
     * Deletes an existing PayIncome model.
    ...
    /**
     * Finds the PayIncome model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return PayIncome the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PayIncome::findOne(['id' => $id])) !== null) {
            return $model;
        }
        ...
        if (!empty($request)) {
            if (!empty($request['a_min']) && !empty($request['a_max'])) {
                if (is_numeric($request['a_min']) && is_numeric($request['a_max']))
                    $query->andFilterWhere(['between', 'incIncome', $request['a_min'], $request['a_max']]);
            }
        } else {
            $dataProvider = null;
        }
